...
2. **Simplify mini-pass** — apply `.claude/skills/code-simplification.md` to *only the staged/unstaged diff for this commit*. Branch-base for the diff is `git merge-base origin/main HEAD` (avoids confusion when a prior PR off the same branch was squash-merged). Targets:
   ...
   - **Comment audit** — for each comment in this commit's diff: (a) explains *what* → delete; explains *why* → keep. (b) Block >3 lines → can one tight sentence carry the *why*? If yes, trim. (c) KDoc on a private method whose name already says it → delete. (d) Same intent already captured in the PR decision log → delete here; the log is the single source of truth. (e) Inline comment narrating a multi-line block → extract a named helper instead; the name replaces the comment. See `code-simplification.md` §"Naming and readability" for the full table.
